...
impl Trajectory {
    ...
    fn randomize(&mut self, n_times: usize) {
        ...
        let epsilon = 15.0;
        let uniform = rand::distributions::Uniform::<f64>::new_inclusive(-epsilon, epsilon);
        let uniform_prob = rand::distributions::Uniform::<f64>::new_inclusive(0.0, 1.0);
        for _ in 0..n_times {
            for i in 0..N {
                let old_x = self.x[i];
