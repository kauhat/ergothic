...
impl Exporter for MongoExporter {
    fn export(&mut self, measures: &Measures) -> Result<(), ExportError> {
        let serialized_data = mongodb::bson::to_bson(measures).expect("Serialization error");
        if let mongodb::bson::Bson::Document(doc) = serialized_data {
            ...
            match self.collection.insert_one(doc, options) {
                Ok(res) => {
                    if let mongodb::bson::Bson::ObjectId(id) = res.inserted_id {
                        info!(
                            "Measurements flushed to {}, obj_id={}",
                            self.formatted_addr,
                            id.to_hex()
                        );
                        Ok(())
                        ...
                        Err(ExportError(
                            "MongoDB didn't return a new object ID.".to_string(),
                        ))
                    }
                }
            ...
            panic!(
                "Serialization error: expected Bson::Document, found {}",
                &serialized_data
            );
        }
    }
